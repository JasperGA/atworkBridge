...
RUN if [[ "$ADD_custom_msgs" = "1" ]]; then                             \
      ...
      source /opt/ros/humble/setup.bash;                                \
      time colcon build --cmake-args -DCMAKE_BUILD_TYPE=Release;        \
      #Compile my message types						\
      # ROS1								\			
      #cd /atwork_bridge_msgs/atwork_bridge_msgs_ros1;			\
      #unset ROS_DISTRO;                                                 \
      #time colcon build --cmake-args -DCMAKE_BUILD_TYPE=Release;        \
      ## Compile ROS2:                                                   \
      #cd /atwork_bridge_msgs/atwork_bridge_msgs_ros2;                   \
      #source /opt/ros/humble/setup.bash;                                \
      #time colcon build --cmake-args -DCMAKE_BUILD_TYPE=Release;        \
    fi
    
RUN mkdir -p /ros1_ws_msgs/src
COPY atwork_bridge_msgs/atwork_bridge_msgs_ros1 /ros1_ws_msgs/src/atwork_bridge_msgs
RUN cd /ros1_ws_msgs && unset ROS_DISTRO && \
     time colcon build --cmake-args -DCMAKE_BUILD_TYPE=Release
     
RUN mkdir -p /ros2_ws_msgs/src
COPY atwork_bridge_msgs/atwork_bridge_msgs_ros2 /ros2_ws_msgs/src/atwork_bridge_msgs
RUN cd /ros2_ws_msgs && \
     source /opt/ros/humble/setup.bash && \
     time colcon build --cmake-args -DCMAKE_BUILD_TYPE=Release
...
RUN                                                                             \
    ...
    #-------------------------------------                                      \
    #source /opt/ros/humble/setup.bash;                                          \
    source /ros1_ws_msgs/install/setup.bash;					\
    source /opt/ros/humble/setup.bash;						\
    source /ros2_ws_msgs/install/local_setup.bash;				\ 
    #                                                                           \
    #-------------------------------------                                      \
